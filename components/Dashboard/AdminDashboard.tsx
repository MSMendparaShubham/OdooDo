...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ users, attendance, leaves, onSelectUser, onApproveLeave, onRejectLeave }) => {
  ...
  const totalLiability = mockApi.getCompanyLiability();
  ...
  const stats = [
    ...
    { label: 'Total Monthly Liability', value: `$${totalLiability.toLocaleString()}`, icon: 'fa-receipt', color: 'text-rose-600', sub: 'Gross Corp Cost' },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((item, i) => (
          <div key={i} className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
            ...
            {item.sub && (
              <p className="text-[10px] text-gray-400 dark:text-slate-500 font-bold uppercase tracking-widest mt-2">{item.sub}</p>
            )}
          </div>
        ))}
      </div>
