  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newUserTemplate: Omit<User, 'id'> = {
        ...
        phone: '',
        address: '',
        profilePic: `https://api.dicebear.com/7.x/avataaars/svg?seed=${formData.employeeId}`,
        department: 'General',
        position: 'Associate',
      ...
      const updatedUsers = mockApi.registerUser(newUserTemplate);
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden p-8 md:p-12 transition-theme">
        <div className="text-center mb-8">
           ...
           <p className="text-gray-500 dark:text-slate-400 mt-2 font-medium">Provision your OdooDo corporate identity</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full md:col-span-2 py-4 bg-blue-600 text-white rounded-2xl font-bold text-lg shadow-xl shadow-blue-200 dark:shadow-blue-900/20 hover:bg-blue-700 transition-all mt-4 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50"
          >
